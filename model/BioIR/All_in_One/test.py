...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_name', type=str, default="epoch=43-val_psnr=21.80.ckpt", help='checkpoint save path')
    ...
    net  = BioIRModel().load_from_checkpoint(ckpt_path).cuda()
    net.eval()
    ...
    if testopt.mode == 0:
    ...
    elif testopt.mode == 1: #260126_nbc8_edit
        ...
        print('Start testing deraining...')
        if os.path.exists(testopt.derain_path):
            derain_set = DerainDehazeDataset(testopt, addnoise=False, sigma=0, task="derain")
            test_Derain_Dehaze(net, derain_set, task="derain")
        else:
            print(f"Path not found: {testopt.derain_path}")

    elif testopt.mode == 2: #260126_nbc8_edit
        ...
        print('Start testing Dehaze...')
        if os.path.exists(testopt.dehaze_path):
            dehaze_set = DerainDehazeDataset(testopt, addnoise=False, sigma=0, task="dehaze")
            test_Derain_Dehaze(net, dehaze_set, task="dehaze")
        else:
            print(f"Path not found: {testopt.dehaze_path}")
    ...
    elif testopt.mode == 5: #260126_nbc8_edit
        # for testset,name in zip(denoise_tests,denoise_splits) :
        #     print('Start {} testing Sigma=15...'.format(name))
        #     test_Denoise(net, testset, sigma=15)
    ...
    elif testopt.mode == 6: #260126_nbc8_edit
        # for testset,name in zip(denoise_tests,denoise_splits) :
        #     print('Start {} testing Sigma=15...'.format(name))
        #     test_Denoise(net, testset, sigma=15)
