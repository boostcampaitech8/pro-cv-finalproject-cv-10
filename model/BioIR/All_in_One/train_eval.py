...
class BioIRModel(pl.LightningModule):
    ...
    def eval_datasets(self):
        ...
        if any('denoise' in t for t in self.opt.de_type): # block edit 260120_nbc8 
        ...
        # derain
        if 'derain' in self.opt.de_type: # nbc8_edit
            ...
            if os.path.exists(self.opt.derain_path):
                self.derain_set = DerainDehazeDataset(self.opt, addnoise=False, sigma=15)
            else:
                print(f"[Warning] Derain test path not found: {self.opt.derain_path}")

        # self.opt.dehaze_path = self.opt.dehaze_path #??
        if 'dehaze' in self.opt.de_type:
            # self.dehaze_set = DerainDehazeDataset(self.opt,addnoise=False,sigma=15)
            if os.path.exists(self.opt.dehaze_path):
                self.dehaze_set = DerainDehazeDataset(self.opt, addnoise=False, sigma=15)
            else:
                print(f"[Warning] Dehaze test path not found: {self.opt.dehaze_path}")
    ...
    def test_Derain_Dehaze(self, dataset, task="derain"):
        ...
        with torch.no_grad():
            ...
            self.log("val_psnr", psnr.avg, sync_dist=True)
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=50, help='maximum number of epochs to train the total model.') #260120_nbc8_edit 150->50
    parser.add_argument('--batch_size', type=int,default=8,help="Batch size to use per GPU") #260125_nbc8_edit 16->8
    ...
    parser.add_argument('--num_workers', type=int, default=4, help='number of workers.') #260120_nbc8_edit 16->4
    ...
    parser.add_argument("--num_gpus",type=int,default= 1, help = "Number of GPUs to use for training") #260120_nbc8_edit 2->1
    ...
    checkpoint_callback = ModelCheckpoint( # nbc8_edit
    ...
    model = BioIRModel(opt, eval_interval=1) #nbc8_edit eval_interval=5->1
    trainer = pl.Trainer(
