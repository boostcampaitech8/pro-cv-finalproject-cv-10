...
class BioIRTrainDataset(Dataset):
    def __init__(self, args):
        super(BioIRTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def _init_hazy_ids(self): #nbc8_edit
        # temp_ids = []
        ...
        self.hazy_ids = []
        # args.dehaze_dir = "data/Train/Dehaze/"
        input_dir = os.path.join(self.args.dehaze_dir, 'input')

        if os.path.exists(input_dir):
            files = os.listdir(input_dir)
            temp_ids = [os.path.join(input_dir, x) for x in files]
            self.hazy_ids = [{"clean_id": x, "de_type": 4} for x in temp_ids]
        else:
             print(f"[Error] Dehaze input not found: {input_dir}")
    ...
    def _init_rs_ids(self): #nbc8_edit
        # temp_ids = []
        ...
        # self.rs_ids = self.rs_ids * 12 # 260120_nbc8_edit * 120 -> * 12
        ...
        self.rs_ids = []
        # args.derain_dir = "data/Train/Derain/"
        input_dir = os.path.join(self.args.derain_dir, 'input')
        
        if os.path.exists(input_dir):
            files = os.listdir(input_dir)
            temp_ids = [os.path.join(input_dir, x) for x in files]
            self.rs_ids = [{"clean_id": x, "de_type": 3} for x in temp_ids]
            self.rs_ids = self.rs_ids * 2 # 260125_nbc8_edit #260129_nbc8_edit 4->2
        else:
            print(f"[Error] Derain input not found: {input_dir}")
    ...
    def _get_gt_name(self, rainy_name): # 260120_nbc8_edit
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            #     clean_img = crop_img(np.array(Image.open(clean_name).convert('RGB')), base=16)
            if de_id == 3 or de_id == 4: #nbc8_edit
                input_path = sample["clean_id"] 
                
                degrad_img = crop_img(np.array(Image.open(input_path).convert('RGB')), base=16)
                clean_path = input_path.replace('/input/', '/target/').replace('\\input\\', '\\target\\')
                clean_img = crop_img(np.array(Image.open(clean_path).convert('RGB')), base=16)
                clean_name = input_path.split('/')[-1].split('.')[0]
            elif de_id == 5:
...
class DerainDehazeDataset(Dataset):
    ...
    def _init_input_ids(self): #nbc8_edit
        if self.task_idx == 0:
            self.ids = []
            ...
            self.ids = []
            name_list = os.listdir(self.args.dehaze_path + 'hazy/')
            self.ids += [self.args.dehaze_path + 'hazy/' + id_ for id_ in name_list]
    ...
    def _get_gt_path(self, degraded_name): #nbc8_edit
        if self.task_idx == 0:
            gt_name = degraded_name.replace("rainy", "gt")
        elif self.task_idx == 1:
            # dir_name = degraded_name.split("input")[0] + 'target/'
            # name = degraded_name.split('/')[-1].split('_')[0] + '.png'
            # gt_name = dir_name + name
            gt_name = degraded_name.replace("hazy", "gt")
        elif self.task_idx == 2:
